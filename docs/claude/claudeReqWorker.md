...
## 13. Phase 2-1 / 2-2 완료 현황 (2026-03 기준)

### 13-1. Phase 2-1 (통신 테스트 스텁) ✅ 완료

`cali-worker-dev` (`C:\BadaDev\cali-worker-dev`) 기준으로 구현 완료.

| 파일 | 내용 |
|---|---|
| `api/JobController.java` | `POST /api/jobs/execute` → 202 즉시 반환 + `@Async` 비동기 위임 |
| `client/CaliApiClient.java` | 배치 조회 + 콜백 전송 + sheetSetting + fillData 조회 메서드 포함 |
| `config/ApiKeyInterceptor.java` | `X-Worker-Api-Key` 헤더 검증 |
| `config/WebConfig.java` | 인터셉터 등록 |
| `config/AsyncConfig.java` | `@EnableAsync` 스레드풀 설정 |
| `service/JobServiceImpl.java` | 비동기 배치 처리 (스텁 → 실제 구현으로 전환 완료) |
| `service/StorageService.java` | S3 샘플 다운로드 + 완성 파일 업로드 |
| `service/ExcelService.java` | Apache POI 기반 셀 삽입 |
| `dto/WorkerDataDTO.java` | CALI API 응답 역직렬화용 DTO (SheetSettingRes, ReportFillDataRes) |

### 13-2. Phase 2-2 (실제 성적서 작성) ✅ 완료

**CALI 측 추가 구현:**

| 파일 | 내용 |
|---|---|
| `dto/WorkerDataDTO.java` | SheetFieldSetting, SheetSettingRes, ReportFillDataRes |
| `service/WorkerDataServiceImpl.java` | Env JSON 파싱 + 6개 테이블 조합 |
| `api/WorkerCallbackController.java` | `GET /api/worker/env/sheet-setting`, `GET /api/worker/reports/{id}/fill-data` 추가 |
| `service/ReportJobBatchServiceImpl.java` | SUCCESS 콜백 시 file_info 생성 + `batchRepository.save()` 명시적 merge |

**워커 측 추가 구현:**

- `StorageService`: S3 업로드 경로 → `{rootDir}/report/{reportId}/origin.xlsx` (고정 파일명)
- `ExcelService`: POI `CellReference` 파싱 + 리플렉션 기반 필드 추출 + 날짜/이름 포맷터
- `JobServiceImpl`: 스텁 제거 → 실제 4단계 처리 (sheetSetting → downloadSample → fillReport → uploadReport)

**버그 수정 이력:**

| 버그 | 원인 | 수정 |
|---|---|---|
| "샘플 파일 정보가 없습니다. sampleId=1" | `fileInfoRepository.findById(sampleId)` 호출 — sampleId는 `file_info.id`가 아닌 `sample.id` | `findByRefTableNameAndRefTableIdAndIsVisible("sample", sampleId, YnType.y)` 로 변경 |
| 성적서 작성 완료 후 배치 status가 DB에 반영 안 됨 | `softDeleteVisibleByRefAndDir`의 `clearAutomatically=true`로 batch 엔티티 detached | SUCCESS 처리 후 `batchRepository.save(batch)` 명시적 추가 |

---

## 14. 실무자결재 페이지 파일 다운로드 구현 (2026-03 기준)

### 14-1. 구현 내용

실무자결재(`workApproval`) 그리드에 **원본 / EXCEL / PDF 다운로드 열** 추가.

**S3 파일 저장 경로 규칙 (고정 파일명 방식):**

| 파일 종류 | S3 objectKey |
|---|---|
| 성적서 원본 | `{rootDir}/report/{reportId}/origin.xlsx` |
| 결재 완료 EXCEL | `{rootDir}/report/{reportId}/signed.xlsx` |
| 결재 완료 PDF | `{rootDir}/report/{reportId}/signed.pdf` |

> 기존 CALI 파일 다운로드 로직(`downloadFile(fileId)`)은 `{dir}{fileId}.ext` 규칙을 사용하므로,
> 성적서 파일은 **별도 전용 API**를 사용한다.

**관련 API:**
- `GET /api/file/report/{reportId}/{fileType}` — fileType: `origin` | `signed_xlsx` | `signed_pdf`

**변경 파일:**

| 파일 | 변경 내용 |
|---|---|
| `service/FileServiceImpl.java` | `downloadReportFile(reportId, fileType)` 메서드 추가 |
| `api/FileController.java` | `GET /api/file/report/{reportId}/{fileType}` 엔드포인트 추가 |
| `repository/ReportRepository.java` | 서브쿼리 name 조건 변경: `report_origin`→`origin`, `report_excel`→`signed_xlsx`, `report_pdf`→`signed_pdf` |
| `service/ReportJobBatchServiceImpl.java` | file_info originName=`origin.xlsx`, name=`origin` |
| `cali-worker/service/StorageService.java` | 업로드 파일명 `report_origin.xlsx` → `origin.xlsx` |
| `static/js/cali/workApproval.js` | 다운로드 렌더러: `props.grid.getRow(props.rowKey).id`로 reportId 조회 + `/api/file/report/{id}/{fileType}` 호출 |

**그리드 컬럼 배치:**
```
... | 기술책임자 | 원본 | 업로드 | EXCEL | PDF |
```
- `원본`: 업로드 바로 왼쪽 (성적서작성 완료 시 표시)
- `EXCEL` / `PDF`: 업로드 오른쪽 (결재 완료 시 표시 — 현재 미구현, 컬럼만 존재)

### 14-2. 완료 후 그리드 자동 리로드

성적서작성 완료 확인 버튼 클릭 시 `workApproval` 그리드가 자동 리로드되도록 커스텀 이벤트 연결.

- `reportWrite.js`: 배치 완료 메시지 확인 후 `$(document).trigger('reportWriteCompleted')` 실행
- `workApproval.js`: `$(document).on('reportWriteCompleted.workApproval', ...)` 핸들러에서 현재 페이지 유지 리로드

---

## 15. 남은 후속 작업

### 15-1. 실무자결재 기능 구현 (WORK_APPROVAL)

현재 '업로드' 열에서 엑셀 파일을 선택하면 "구현 준비중" 토스트가 표시됨.

**구현 목표:**
1. 실무자가 성적서 원본(`origin.xlsx`)을 다운로드 후 서명 작성 → 재업로드
2. 또는 워커가 자동으로 서명/도장 이미지를 삽입하여 `signed.xlsx` / `signed.pdf` 생성
3. `report.work_status` → SUCCESS, `work_datetime` 업데이트
4. `file_info` 에 `name='signed_xlsx'`, `name='signed_pdf'` 레코드 생성
5. 결재 완료 시 EXCEL / PDF 다운로드 아이콘 표시
...
### 15-2. 기술책임자결재 기능 구현 (MANAGER_APPROVAL)

실무자결재 완료 후 기술책임자 결재 단계. 동일한 워커 파이프라인 활용 예정.
...
- 성적서작성 FAIL 시 재시도 버튼 (현재 없음)
- `retry_count` 컬럼 활용

### 15-4. 업로드 컬럼 (실무자결재) 실제 처리 연결

`workApproval.js`의 `handleWorkApprovalUploadFile()` 함수가 현재 "구현 준비중" 상태.
실무자가 서명된 파일을 직접 업로드하는 방식으로 처리할 경우 여기에 연결 필요.

---

## 16. WORK_APPROVAL(실무자결재) 구현 설계 (2026-03-21 확정)

### 16-1. 서명 이미지 삽입 (워커 처리)

- 서명 이미지는 **워커 서버**에서 origin.xlsx에 삽입
- 엑셀 내 shape 이름 기준으로 삽입 위치 결정
  - WORK_APPROVAL: `writer_sign` shape → 실무자 서명 이미지
  - MANAGER_APPROVAL: `manager_sign` shape → 기술책임자 서명 이미지
- 같은 이름의 shape가 시트 내 여러 개 있으면 **모두** 삽입

**서명 이미지 소스:**
- `file_info` 테이블에서 조회: `ref_table_name='member'`, `ref_table_id={memberId}`
- `member.img` 컬럼은 사용하지 않음 (무시)
- 스토리지 objectKey: file_info의 stored_path 참조

**전달 방식 (확정: CALI 페이로드에 포함):**
- CALI 서버가 트리거 시점에 서명 이미지 objectKey를 조회하여 `TriggerReq`에 포함
- `TriggerReq`에 추가할 필드: `workerSignImgKey` (실무자 서명 이미지 스토리지 objectKey)
- 이유: 워커가 단독으로 동작 가능 (멀티테넌트 원칙 유지), 트리거 시점에 결재자 확정

### 16-2. PDF 변환 (LibreOffice headless)

**사용 라이브러리:** LibreOffice headless
- `soffice --headless --convert-to pdf` 명령 실행 방식
- Windows 로컬: LibreOffice 설치 후 `soffice.exe --headless` 로 구동 가능
- WSL2/운영 서버: `sudo apt install libreoffice` 후 동일하게 사용 가능

**변환 대상 시트 및 조건:**

| report_lang | 변환할 시트 |
|---|---|
| KR | 성적서(갑)(국문), 성적서(을)(국문) |
| EN | 성적서(갑)(영문), 성적서(을)(영문) |
| BOTH | 성적서(갑)(국문), 성적서(을)(국문), 성적서(갑)(영문), 성적서(을)(영문) |

- '데이터' 시트는 변환 대상 제외
- 해당 시트들을 하나의 PDF로 합본하여 업로드 (`signed.pdf`)
- `report_lang` 값은 TriggerReq 또는 fill-data API 응답에 포함하여 워커에 전달

**LibreOffice 특정 시트만 변환 방법:**
1. POI로 대상 시트만 남긴 임시 xlsx 생성 (또는 시트 숨김/표시 조작)
2. LibreOffice headless로 pdf 변환
3. 여러 시트 → 합본 PDF는 LibreOffice가 자동 처리

### 16-3. 업로드 열 UI 변경 (workApproval.js / gridClass.js)

**현재:** `UploadCellRenderer` — EXCEL 업로드 버튼 항상 표시 (width: 80)

**변경 후:**
- `originFileId`가 **없으면** (성적서 미작성): EXCEL 업로드 버튼만 표시 (현재와 동일)
- `originFileId`가 **있으면** (성적서 작성 완료): **EXCEL 업로드 아이콘 + 결재 아이콘** 나란히 표시
  - 셀 너비는 그대로 (80px), 텍스트 없이 아이콘만 표시
  - 결재 아이콘: `bi-pen-fill` 또는 `bi-check2-circle` (결재 의미)

**'결재' 클릭 동작:**
- 성적서작성과 동일한 Polling UI (진행상황 패널 + 폴링)
- 워커 트리거: WORK_APPROVAL 타입 배치 생성 → `POST /api/report/jobs/batches`
- 워커 처리: origin.xlsx 다운로드 → `writer_sign` shape에 서명 이미지 삽입 → PDF 변환 → signed.xlsx + signed.pdf 업로드

**변경 대상 파일:**
- `gridClass.js` — `UploadCellRenderer`에서 `originFileId` 참조 방식 변경
  - 현재 렌더러는 `props.value`만 받음 → 행 전체 데이터 접근이 필요하므로 구현 방식 검토 필요
  - 또는 `workApproval.js`에서 별도 렌더러 클래스 정의 권장 (UploadCellRenderer 공용 유지)
- `workApproval.js` — 결재 버튼 클릭 핸들러 및 배치 생성/폴링 로직 추가

### 16-4. 파일 다운로드 파일명 변경

**요구사항:** `{성적서번호}.xlsx` / `{성적서번호}.pdf` 형태로 다운로드

**구현 방식 (확정: 쿼리파라미터 전달 — DB 조회 없음):**
- 프론트가 그리드 데이터에서 이미 `reportNum`을 알고 있음
- API: `GET /api/file/report/{reportId}/{fileType}?reportNum={reportNum}`
- 백엔드 `FileController.reportFileDownload()` — `@RequestParam String reportNum` 추가
- `FileServiceImpl.downloadReportFile()` — `reportNum` 파라미터 추가, Content-Disposition에 적용
- reportNum이 null이거나 빈값이면 기존 고정명(`origin.xlsx` 등)으로 fallback

**변경 대상 파일:**
- `api/FileController.java` — `@RequestParam` 추가
- `service/FileServiceImpl.java` — 파라미터 추가 + Content-Disposition 파일명 적용
- `static/js/cali/workApproval.js` — `downloadReportFile()` 함수에 reportNum 쿼리파라미터 추가